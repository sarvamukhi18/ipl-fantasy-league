...
def get_bets_live():
    ...
    try:
        ...
        # Return empty shell if sheet is totally blank or missing
...
if not active_matches.empty:
    ...
    if current_edits >= 2:
        # FIX: Ensure we use the right variable name here
        ...
        **Locked Prediction:**
        ...
        if submitted:
            ...
            # Logic: Update existing row or add new
            ...
            if not live_bets[row_mask].empty:
                ...
                live_bets.at[idx, 'EditCount'] = 2 # Lock it
            else:
...
with t2:
    ...
    if not bets_df.empty:
        # We only show columns that are actually interesting to the players
        display_cols = ["Player", "MatchID", "Predicted Team", "Multiplier", "Predicted MOTM", "Timestamp"]
        # Filter again just to be safe from 'nan' rows in the table
        ...
        st.info("No bets placed yet.")import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd
from datetime import datetime
import time
...
def get_bets_live():
    try:
        # Load live for the "Upsert" logic
        b_df = conn.read(worksheet="Bets", ttl=0).dropna(how='all').fillna('')
        if not b_df.empty:
            b_df['MatchID'] = b_df['MatchID'].astype(str).str.split('.').str[0].str.strip()
            b_df['Player'] = b_df['Player'].astype(str).str.strip()
            # Ensure 'EditCount' column exists to track modifications
            if 'EditCount' not in b_df.columns:
                b_df['EditCount'] = 0
            b_df['EditCount'] = pd.to_numeric(b_df['EditCount'], errors='coerce').fillna(0).astype(int)
        else:
            # Create structure if sheet is empty
            b_df = pd.DataFrame(columns=["Player", "MatchID", "Predicted Team", "Multiplier", "Predicted MOTM", "EditCount", "Timestamp"])
        return b_df
    except:
...
if not active_matches.empty:
    ...
    # Find if a bet already exists for this specific player/match
    existing_bet_mask = (bets_df['Player'] == current_user) & (bets_df['MatchID'] == m_id)
    user_bet = bets_df[existing_bet_mask]

    # Track edits (0 = New, 1 = Modified, 2 = Locked)
    current_edits = int(user_bet['EditCount'].values[0]) if not user_bet.empty else 0

    if current_edits >= 2:
        bet_data = user_bet.iloc[0]
        st.sidebar.success(f"✅ Bet Finalized for {current_user}")
        st.sidebar.markdown(f"**Winner:** {bet_row['Predicted Team']}  \n**Mult:** x{bet_row['Multiplier']}")
        st.sidebar.warning("No more changes allowed.")
    else:
        with st.sidebar.form("bet_form"):
            label = "Submit First Bet" if current_edits == 0 else "Modify Bet (Last Chance!)"
            st.subheader(label)
            p_team = st.radio("Who wins?", [m_info['Team 1'], m_info['Team 2']])
            p_mult = st.select_slider("Multiplier", options=[1, 2, 3])
            p_names = sorted(players_df['Name'].tolist()) if not players_df.empty else ["N/A"]
            p_motm = st.selectbox("MOTM", p_names)
            submitted = st.form_submit_button("Lock In")

        if submitted:
            # Atomic update: get fresh data before saving
            live_bets = get_bets_live()
            new_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            
            # Logic: If row exists, update it. If not, append it.
            mask = (live_bets['Player'] == current_user) & (live_bets['MatchID'] == m_id)
            
            if not live_bets[mask].empty:
                # UPDATE EXISTING ROW
                idx = live_bets[mask].index[0]
                live_bets.at[idx, 'Predicted Team'] = p_team
                live_bets.at[idx, 'Multiplier'] = p_mult
                live_bets.at[idx, 'Predicted MOTM'] = p_motm
                live_bets.at[idx, 'Timestamp'] = new_timestamp
                live_bets.at[idx, 'EditCount'] = 2  # Mark as locked now
            else:
                # INSERT NEW ROW
                new_row = pd.DataFrame([{
                    "Player": current_user, "MatchID": m_id, "Predicted Team": p_team,
                    "Multiplier": p_mult, "Predicted MOTM": p_motm,
                    "EditCount": 1, "Timestamp": new_timestamp
                }])
                live_bets = pd.concat([live_bets, new_row], ignore_index=True)
...
with t2:
    st.subheader("Global Bets")
    # Display the table without the internal EditCount column for users
    if not bets_df.empty:
        display_cols = ["Player", "MatchID", "Predicted Team", "Multiplier", "Predicted MOTM", "Timestamp"]
        st.dataframe(bets_df[display_cols].sort_index(ascending=False), hide_index=True)
