...
try:
    ...
    st.error("Google API is busy. Please refresh in 30 seconds.")
    st.stop()

# 2. Run Automation
matches_df, needs_update = sync_results(matches_df)
if needs_update:
    # Small pause to prevent hitting Google limits
    time.sleep(1) 
    conn.update(worksheet="Matches", data=matches_df)
    st.toast("Match results updated!", icon="✅")

# 3. Player List Fallback
try:
...
# --- SIDEBAR & MAIN PAGE (Same as before) ---
...
if not upcoming_df.empty:
    ...
    st.sidebar.info("No upcoming matches.")
