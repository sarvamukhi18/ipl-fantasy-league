...
if not upcoming_df.empty:
    ...
    # --- CHECK ATTEMPT COUNT ---
    # We filter only for rows that have a real Predicted Team (ignores ghost/empty rows)
    ...
    if attempts >= 2:
        ...
        **Final Prediction:**
        ...
        # ALLOW SUBMISSION (New or Edit)
        form_label = "Submit First Bet" if attempts == 0 else "Modify My Bet (Last Chance!)"
        if attempts == 1:
            st.sidebar.info("You have already placed one bet. Submitting again will lock your choice permanently.")
...
# Show only valid bets in the table
st.dataframe(bets_df[bets_df['Predicted Team'] != ""].tail(20), use_container_width=True, hide_index=True)import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd
from datetime import datetime
import requests
