...
if not upcoming_df.empty:
    ...
    match_id = upcoming_df[upcoming_df['display_name'] == selected_match_display]['MatchID'].values[0]
    match_info = matches_df[matches_df['MatchID'] == match_id].iloc[0]

    # --- CHECK FOR EXISTING BET ---
    # We check if this player has already bet on THIS specific MatchID
    existing_bet = bets_df[(bets_df['Player'] == current_player) & (bets_df['MatchID'] == match_id)]

    if not existing_bet.empty:
        st.sidebar.success(f"✅ Bet Confirmed for Match {match_id}")
        st.sidebar.info(f"**Your Pick:** {existing_bet.iloc[0]['Predicted Team']}  \n"
                        f"**Multiplier:** x{existing_bet.iloc[0]['Multiplier']}  \n"
                        f"**MOTM:** {existing_bet.iloc[0]['Predicted MOTM']}")
        st.sidebar.warning("Bets cannot be changed once submitted.")
    else:
        # Show form only if no bet exists
        ...
        if submit:
            new_bet = pd.DataFrame([{
                "Player": current_player, "MatchID": match_id, "Predicted Team": pred_team,
