...
st.markdown("""
    ...
    .match-card, .result-card, .bet-summary-card {
        background: #121212;
...
with st.container():
    ...
    with c2:
        ld_df = conn.read(worksheet="Leaderboard", ttl="1h").fillna(0)
        score_val = ld_df[ld_df['Player'] == current_user]['Total'].values
        st.metric("POINTS", f"{score_val[0] if len(score_val) > 0 else 0}")
...
with col_main:
    ...
    tab1, tab2 = st.tabs(["⚡ FIXTURES", "🏁 RESULTS"])
    ...
    with tab1:
        ...
        if todays_matches.empty:
            ...
            for i, (idx, match) in enumerate(todays_matches.iterrows()):
                ...
                # Filter bets for THIS match
                match_bets = bets_df[bets_df['MatchID'] == m_id]
                user_bet = match_bets[match_bets['Player'] == current_user]
                ...
                if not is_locked:
                    with st.form(f"form_{m_id}"):
                        ...
                        if st.form_submit_button("SUBMIT"):
                            ...
                            # Fresh pull to avoid overwrite issues
                            current_bets = get_bets_live()
                            mask = (current_bets['Player'] == current_user) & (current_bets['MatchID'] == m_id)
                            
                            if not current_bets[mask].empty:
                                current_bets.loc[mask.idxmax(), ['Predicted Team', 'Multiplier', 'Predicted MOTM', 'Timestamp']] = [p_team, p_mult, p_motm, ts]
                            else:
                                ...
                                current_bets = pd.concat([current_bets, new_row], ignore_index=True)
                            
                            conn.update(worksheet="Bets", data=current_bets)
                            st.cache_data.clear()
                            st.toast(f"✅ Bet Placed for {p_team}!", icon='🏏')
                            st.success(f"Confirmation: Your bet for {p_team} (x{p_mult}) has been saved.")
                            time.sleep(1)
                            st.rerun()

                # --- SHOW RECENT BETS FOR THIS MATCH ---
                if not match_bets.empty:
                    with st.expander(f"📊 View All Predictions ({len(match_bets)})", expanded=is_locked):
                        for _, bet in match_bets.iterrows():
                            st.markdown(f"**{bet['Player']}**: {bet['Predicted Team']} (x{bet['Multiplier']}) • MOTM: {bet['Predicted MOTM']}")
        ...
        for _, row in completed.iterrows():
            st.markdown(f"""
                <div class="result-card">
                    ...
                    <div style="margin-top: 5px; color: white;">Winner: <b style="color: #34d399;">{row['Winner']}</b> • MOTM: <b>{row['MOTM']}</b></div>
                </div>
            """, unsafe_allow_html=True)

with col_side:
    st.markdown("<h3 style='color: white;'>🏆 STANDINGS</h3>", unsafe_allow_html=True)
    st.dataframe(ld_df[['Player', 'Total']].sort_values(by='Total', ascending=False), hide_index=True, use_container_width=True)
