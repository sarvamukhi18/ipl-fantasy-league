...
st.markdown("""
    ...
    .main-header {
        ...
        margin-top: -60px; /* Pulls it up to remove the gap */
    ...
    .status-open { background: #d32f2f; color: white; border: 1px solid #ff5252; }
    .status-closed { background: #333333; color: #999999; }
...
# --- 2. CONSOLIDATED TOP SECTION ---
st.markdown("""
    <div class="main-header">
        <h1 style="color: white; margin: 0; font-family: 'Arial Black'; letter-spacing: -1px;">🏏 IPL FANTASY LEAGUE</h1>
    </div>
""", unsafe_allow_html=True)

# Profile Selection (First thing on mobile)
with st.container():
    c1, c2, c3 = st.columns([1.5, 1, 1])
    with c1:
        current_user = st.selectbox("IDENTIFY PLAYER", ["S", "G", "T", "Shy", "Y", "D", "A"])
    with c2:
        ...
        st.metric("YOUR POINTS", f"{score_val}", delta_color="normal")
    with c3:
...
with col_main:
    ...
    if todays_matches.empty:
        st.info("No matches today. Check the schedule below.")
    else:
        for i, (idx, match) in enumerate(todays_matches.iterrows()):
            ...
            st.markdown(f"""
                <div class="match-card">
                    ...
                    <div style="color: #999; font-size: 0.7em; margin-bottom: 5px;">MATCH {m_id} • {match['Venue']}</div>
                    <div class="team-name" style="font-size: 1.5em;">
                        {match['Team 1']} <span style="color: #d32f2f; font-size: 0.8em;">VS</span> {match['Team 2']}
                    </div>
                </div>
            """, unsafe_allow_html=True)

            if is_locked:
                if match_completed: st.error(f"Winner: {match['Winner']}")
                if has_bet: st.info(f"Your Prediction: {user_bet.iloc[0]['Predicted Team']} (x{user_bet.iloc[0]['Multiplier']})")
            else:
                with st.expander("SET PREDICTION", expanded=not has_bet):
                    with st.form(f"form_{m_id}"):
                        ...
                        with f2:
                            ...
                            p_motm = st.selectbox("MOTM Pick", p_names, key=f"p_{m_id}")
...
with col_side:
    st.markdown("<h3 style='color: white;'>🏆 LEADERBOARD</h3>", unsafe_allow_html=True)
